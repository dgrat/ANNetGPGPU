...
SOMNet<Type, Functor>::SOMNet() {
	this->m_pIPLayer = nullptr;
	this->m_pOPLayer = nullptr;
...
template<class Type, class Functor>
SOMNet<Type, Functor>::SOMNet(const std::vector<uint32_t> &vDimI, const std::vector<uint32_t> &vDimO) {
	this->m_pIPLayer = nullptr;
	this->m_pOPLayer = nullptr;
	m_pBMNeuron = nullptr;
...
void SOMNet<Type, Functor>::TrainHelper(uint32_t i) {
	...
	_centroids[m_pBMNeuron->GetID()] = centr;
...
template<class Type, class Functor>
std::map<uint32_t, Centroid<Type>> SOMNet<Type, Functor>::GetCentroids() {
	return _centroids;
}
