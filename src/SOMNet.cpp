...
void SOMNet::SetSigma0(const float &fVal) {
	if(fVal < 0.f) {
		return;
	}
...
void SOMNet::PropagateBW() {
	...
	for(int i = 0; i < static_cast<int>(m_pOPLayer->GetNeurons().size() ); i++) {		
		...
		float fLambda 		= m_iCycles / log(pNeuron->GetSigma0() ); // time constant
		float fSigmaT 		= m_DistFunction->rad_decay(pNeuron->GetSigma0(), m_iCycle, fLambda);		
		float fLearningRateT 	= m_DistFunction->lrate_decay(pNeuron->GetLearningRate(), m_iCycle, m_iCycles);
		float fDist 		= pNeuron->GetDistance2Neur(*m_pBMNeuron);
		
		if(fDist < fSigmaT) {
			//calculate by how much weights get adjusted ..
			float fInfluence = m_DistFunction->distance(fDist, fSigmaT);
