...
std::vector<SOMExport*> SOMNetGPU::SplitDeviceData() const {
	...
	for(int i = 0; i < iDeviceCount; i++) { 
		...
		for(unsigned int j = 0; j <= iStop-iStart; j++) {
			hvConscience[j] = ((ANN::SOMNeuron*)m_pOPLayer->GetNeuron(j+iStart))->GetConscience();
			hvSigma0[j]     = ((ANN::SOMNeuron*)m_pOPLayer->GetNeuron(j+iStart))->GetSigma0();
			hvLearningRates[j] = ((ANN::SOMNeuron*)m_pOPLayer->GetNeuron(j+iStart))->GetLearningRate();
		}
...
void SOMNetGPU::CombineDeviceData(std::vector<SOMExport*> &SExp) {
	unsigned int iStart 		= 0;
	unsigned int iStop 		= 0;
	unsigned int iSizeOfLayer 	= GetOPLayer()->GetNeurons().size();
	unsigned int iDeviceCount 	= GetCudaDeviceCount();
...
SOMNetGPU::SOMNetGPU() {
	...
	SetLearningRate(m_fLearningRate);
...
void SOMNetGPU::Training(const unsigned int &iCycles, const ANN::TrainingMode &eMode) {
	...
	std::vector<SOMExport*> SOMExp = SplitDeviceData();

	StartTimer();

	printf("Calculate SOM ..\n");
	hostSOMTraining( SOMExp,
		*GetTrainingSet(),
		iCycles,
		m_fConscienceRate,
		*GetDistFunction(),
