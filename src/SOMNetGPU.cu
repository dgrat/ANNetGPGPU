...
std::vector<SOMExport<Type>*> SOMNetGPU<Type, Functor>::SplitDeviceData() const {
	...
	for(int32_t i = 0; i < iDeviceCount; i++) { 
		...
		vRes[i] = new SOMExport<Type>(d2dEdges, d2dPositions, hvSigma0, hvLearningRates);
	}
	return vRes;
}
...
template<class Type, class Functor>
void SOMNetGPU<Type, Functor>::Training(const int32_t &iCycles, const ANN::TrainingMode &eMode) {
	assert(iCycles > 0);
	...
	for(int32_t i = 0; i < SOMExp.size(); i++) {
		SOMExp.at(i)->Clear();
	}
	SOMExp.clear();
...
void SOMNetGPU<Type, Functor>::hostSOMPropagateBW( std::vector<SOMExport<Type>*> &SExp, const int32_t &iPatternID) {
	...
	this->_centroids[BMUExp._unitID] = (BMUExp);
